...
impl Notify {
    ...
    /// Fires every notification type, 3 s apart. Blocks; run on a thread.
    pub fn test_all(&self) {
        let name = "Razer DeathAdder V3 Pro";
        let steps: [&dyn Fn(); 6] = [
            &|| self.device_connected(name),
            &|| {
                self.battery_low(
                    name,
                    28,
            ...
            &|| self.battery_full(name),
            &|| self.device_disconnected(name),
        ];
